  ...
  <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
    <div class="p-6 border-b border-gray-200">
      <form onsubmit="return window.confirm('Yakin ingin submit?')"
        action="{{ route('admin.borrows.update', ['borrow' => $borrow->id]) }}" method="POST"
        ...
        <div class="my-2">
          ...
          <select name="user_id" id="user_id" class="block mt-1 w-full rounded-md form-select focus:border-indigo-600">
            @foreach ($users as $user)
              <option @selected(old('user_id', $borrow->user->id) === $user->id) value="{{ $user->id }}">
                {{ $user->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="my-2">
          ...
          <select name="book_id" id="book_id" class="block mt-1 w-full rounded-md form-select focus:border-indigo-600">
            @foreach ($books as $book)
              <option @selected(old('book_id', $borrow->book->id) === $book->id) value="{{ $book->id }}">
                {{ $book->title }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="my-2">
          ...
          <span class="block mt-2 w-full rounded-md">{{ $borrow->borrowed_at->locale('id')->isoFormat('LLL') }}</span>
        </div>
        ...
        <div class="my-2">
          ...
          <input type="date" value="{{ old('due', $borrow->due->format('Y-m-d')) }}" name="due"
        ...
        @if (!is_null($borrow->returned_at))
          <div class="mt-2">
            ...
            <span class="block mt-2 w-full rounded-md">{{ $borrow->returned_at->locale('id')->isoFormat('LLL') }}</span>
          </div>
        @endif
