  ...
  <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
    <div class="p-6 border-b border-gray-200">
      <form enctype="multipart/form-data" onsubmit="return window.confirm('Yakin ingin submit?')"
        action="{{ route('admin.books.update', ['book' => $book->id]) }}" method="POST"
        ...
        <div class="my-2">
          ...
          <input value="{{ old('writer', $book->writer) }}" name="writer"
            class="block w-full rounded-md form-input focus:border-indigo-600" />
        </div>
        ...
        <div class="my-2">
          ...
          <textarea rows="10" name="description" id="description"
            class="block w-full rounded-md form-textarea focus:border-indigo-600">{{ old('description', $book->description) }}</textarea>
        </div>
